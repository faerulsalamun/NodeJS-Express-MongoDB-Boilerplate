  ...
  app.use((err, req, res, next) => {
    ...
    if (req.api) {
      if (err.message && err.errors === undefined) {
        ...
        const errorData = Utils.showErrorValidationMongo(err);
        const errorStatus = Utils.checkErrorCode(errorData);
        return res.status(errorStatus).send(craft(err.data, errorData.message, errorStatus));
      }
    } else {
